...
// Server Action untuk hapus data imam
...
export async function deleteImam(id) {
  ...
  if (role !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat menghapus data imam.' }
  }

  const unlinkResult = await retrySupabase(() => (
    supabaseAdmin
      .from('jadwal')
      .update({ imam_id: null, status: 'diganti' })
      .eq('imam_id', id)
  ))
  ...
  const { error } = await retrySupabase(() => (
    supabaseAdmin.from('imam').delete().eq('id', id)
  ))
...
export async function deleteUser(userId) {
  ...
  if (currentRole !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat menghapus pengguna.' }
  }

  const sessionUserId = getSessionUserId()
  if (sessionUserId && sessionUserId === userId) {
    return { status: 'error', message: 'Admin tidak bisa menghapus akun yang sedang dipakai login.' }
  }

  if (!userId) {
    return { status: 'error', message: 'Pengguna tidak valid.' }
  }
...
export async function updateUserRole(userId, role) {
  ...
  const allowedRoles = ['user', 'imam', 'admin']
  ...
  if (!data) {
    return {
      status: 'error',
      message: 'Role belum tersimpan. Tambahkan SUPABASE_SERVICE_ROLE_KEY di .env.local atau izinkan admin meng-update tabel users.',
    }
  }
  ...
  return {
    status: 'success',
    message: `Role berhasil diubah menjadi ${data.role}.`,
  }
}
...
export async function createJadwal(prevState, formData) {
  const currentRole = cookies().get('session_role')?.value
  if (currentRole !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat membuat jadwal.', fieldErrors: {} }
  }
  ...
  const payload = {
    ...validated.data,
    tanggal: validated.data.tanggal || null,
    nama_masjid: validated.data.nama_masjid.trim(),
    catatan: validated.data.catatan || '',
    status: validated.data.status || 'terjadwal',
  }

  const { data: existingSchedule, error: duplicateCheckError } = await supabaseAdmin
    .from('jadwal')
    .select('id')
    .eq('malam', payload.malam)
    .ilike('nama_masjid', payload.nama_masjid)
    .limit(1)
    .maybeSingle()
  ...
  if (existingSchedule) {
    return {
      status: 'error',
      message: `Malam ${payload.malam} untuk ${payload.nama_masjid} sudah punya jadwal.`,
      fieldErrors: { malam: 'Jadwal malam ini sudah ada untuk masjid tersebut.' },
    }
  }
...
export async function updateJadwal(prevState, formData) {
  const currentRole = cookies().get('session_role')?.value
  if (currentRole !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat mengubah jadwal.', fieldErrors: {} }
  }

  const scheduleId = formData.get('id')?.toString()
  if (!scheduleId) {
    return { status: 'error', message: 'Jadwal tidak valid.', fieldErrors: {} }
  }
...
export async function updateJadwalStatus(scheduleId, status) {
  const currentRole = cookies().get('session_role')?.value
  if (currentRole !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat mengubah status jadwal.' }
  }

  if (!scheduleId || !allowedScheduleStatuses.includes(status)) {
    return { status: 'error', message: 'Status jadwal tidak valid.' }
  }
...
export async function confirmMyJadwal(scheduleId, nextStatus) {
  ...
  if (currentRole !== 'imam' || !userId) {
    return { status: 'error', message: 'Akses ditolak. Login sebagai imam terlebih dahulu.' }
  }

  if (!['dikonfirmasi', 'berhalangan'].includes(nextStatus)) {
    return { status: 'error', message: 'Status konfirmasi tidak valid.' }
  }

  const { data: imamProfiles, error: imamError } = await supabaseAdmin
    .from('imam')
    .select('id')
    .eq('user_id', userId)
  ...
  if (imamIds.length === 0) {
    return { status: 'error', message: 'Data imam belum terhubung ke akun ini. Isi data imam terlebih dahulu.' }
  }
  ...
  if (!imamIds.includes(schedule.imam_id)) {
    return { status: 'error', message: 'Jadwal ini bukan milik akun imam Anda.' }
  }
...
export async function deleteJadwal(id) {
  const currentRole = cookies().get('session_role')?.value
  if (currentRole !== 'admin') {
    return { status: 'error', message: 'Akses ditolak. Hanya admin yang dapat menghapus jadwal.' }
  }
